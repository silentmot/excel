...
1. **Materials** - 25 materials with properties
2. **MaterialAliases** - 35 alias mappings
3. **Equipment** - 9 equipment types catalog
4. **PersonnelRoles** - 5 manpower categories
5. **Sites** - Facility locations

**Transaction Tables**:

6. **DailyProduction** - Daily crusher output (replaces Master Production rows)
7. **DispatchTransactions** - Individual dispatch events (from Dispatched file)
8. **EquipmentUsage** - Daily equipment hours (from Master/Equipment files)
9. **ManpowerAttendance** - Daily personnel counts (from Master file)
10. **OperationalMetrics** - Other operational KPIs

**Summary Tables**:

11. **InventorySnapshots** - Calculated stock levels (time-series)

### Key Relationships

```
Materials (1) ←→ (N) DailyProduction
Materials (1) ←→ (N) DispatchTransactions
Materials (1) ←→ (N) InventorySnapshots
MaterialAliases (N) ←→ (1) Materials
Equipment (1) ←→ (N) EquipmentUsage
PersonnelRoles (1) ←→ (N) ManpowerAttendance
```
...
### Milestone 2: Data Migration (Week 3-4)
...
### Milestone 4: Data Entry UI (Week 7-8)
...
### Milestone 5: Dashboard & Reporting (Week 9-10)
...
### Milestone 6: Export Features (Week 11)
...
### Milestone 7: Testing & Deployment (Week 12)
...
bunx create-nx-workspace@latest excel-ops --preset=next
